...
\end{figure}
 ...
 E[I] = G* I
 \end{equation}
 If we insert the above equations into each other, we obtain a deviation measure $\Delta d$ for the disparity based on the gradient deviation in the EPI.
...
\end{description}
	...
	\subsection{Using depth-from-refocus as a preestimate for the ST}
		...
		\caption[Depth from focus: depthmaps with Resolution 1]{The depth maps of the scenes (from up to down) \textit{complextestscene, cotton, occlusiontestscene, pens, testscene} is depicted for different depth-from-focus techniques. On the right the ground trhouth is depicted The disparity stepsize is 1.}
...
Resultate nur mit normalisierung
Verschiedene Thresholds testen.
Adaptive Threshold testen.
\subsection{Occlusion awareness segmentation}
Verschiedene thresholds\\

adaptive threshold testen
\\
bester threshold -> verschiedene background/foreground parameter testen (2-parameter python darstellen)\\
