...
\begin{figure}
	...
	\caption[\textit{Alternative Merging}:Parameter dependence]{From left to right: Parameter dependent alternative merging. Up to down: scenes \textit{boxes, cotton, dino, sideboard}. The deviation from ground truth is marked yellow, zero deviation is dark-purple.}
...
\begin{figure}
	...
	\caption[SGM Path explanation]{Left: Minimum cost path along an exemplary scanline. Right: each point gets passed by multiple paths from different directions.}
	\label{fig:sgmpaths}
\end{figure}
...
\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{images/pens_paths}
...
with $|\nabla|(x,y)$ being the centre view colour difference
 ...
 \end{equation}
  $C_\text{\{b,g,r\}}$ are the blue, red and green colour channels of the centre view, respectively. The penalty for a disparity discontinuity is lowered, if the colour intensity difference at this discontinuity is high.
...
In some scenes masks are applied to evaluate the score at different spots in the scene. This way it can be focussed quantitatively on challengin spots such as planar surfaces or discontinuities. Given a mask $M$ the BadPixel score is then given as 
\begin{equation}\label{key}
...
\end{figure}
	...
	One long-term aim of this work is to make the structure tensor pipeline more robust. We achieve this by using the depth-from-refocus method as a pre-estimate, therefore reducing the disparity window at each pixel. The light field is now refocussed by integer disparity steps, resulting in disparity maps that obtain equidistant layers.
	 All disparity maps with integer disparity step size are shown in figure \ref{fig:resultdepthfromfocuspre}, right next to the ground truth integer depth map. From the depth maps one can already guess, that the angular correspondence and the photo consistency cue produce the denser depth estimates.  This again is confirmed if we take a look at the MRE in figure \ref{fig:errorres1all}: The angular correspondence and photo consistency measures result in the lowest MRE, though the angular correspondence cue is about 1.5 times more time-consuming than the photo consistency measure under the same conditions. The process time for all cues is depicted in figure \ref{fig:speeds} on the right.
	 \begin{figure}
	 	\centering
...
As depth from focus techniques didn't work out well, we examine different approaches by directly modifying the ST pipeline, with the aim on solving the occlusion problem while maintaining a dense disparity map.\\
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{images/sandclock_results}
...
\begin{figure}
	\centering
	\includegraphics[width=0.6\linewidth]{images/bilateral_norm_params}
...
In the next step we are trying to detect occlusions in the scan directly via the gradient norm, since a high gradient is a good indicator for an occlusion edge (explained in section \ref{sec:occlusionsegmentation}). The result however is dependent on multiple parameters, two of them are the kernel size of the morphology closing kernel. In figure \ref{fig:threshsegmmorph} (upper row) the metric scores are plotted in function of the y-directional size of the kernel. The x-morphology kernel is held constant to the size of 7. From figure \ref{fig:threshsegmmorph} (lower row) we can extract, that for all scenes we obtain the best result when sticking to a 7-pix x-directional kernel size. As the EPI is only 9 pixels wide, we do not use larger kernels.\\
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{images/newerror_confidence}
...
\begin{figure}
	...
	\caption[\textit{Alternative Merging} with SGM post-processing]{\textit{Alternative Merging} with SGM post-processing Left/Right: MSE/BadPixel score as a function of the merging threshold.}
...
Further we examine the effect of changing the smoothing parameter P1 from equation \ref{eq:recursive1dexp}. This parameter changes the form of penalizing disparity deviations from 2 neighbouring points on a SGM scanline. For very small P1 the penalty is close to being a constant regardless of the disparity deviations, thus no convergence can be reached. In figure \ref{fig:chooselowersgmpprmergeiterationsexppenalty1} this can be seen in the left 
...
%		gradient threshold (0.1) &6.09 & 10.68 & 3.14 & 1.06 & 2.29 \\
		...
		\hline
	\end{tabular}
\caption[Resumee Table]{Comparison of all methods on MSE metric. PPR stands for post-processing. The relevant parameters are found in parentesis.} 
...
An alternative coherence measurement has been developed based on first order error propagation, leading to slightly improved results to a cost of processing time. Occlusion handling with segmenting the EPIs based on the gradient norm showed some good results in scene \textit{cotton}, however it needs clear contrast edges in the scene to identify depth discontinuities. Implementing an \textbf{alternative merging} of x- and y- directional disparity estimations allowed for higher detail resolution and occlusion handling, however occlusions are taken care of with implementing a truncation threshold to the gradient in the EPI. This modification is easily implemented, does not significantly take longer in processing and noticeably improves the MSE for all scenes. \\
...
\\ 
However, while working on the thesis, papers have been published (\cite{luo2017epi}, \cite{shin18epinet}), that present machine-learning based methods to reconstruct depth map from light field data yielding\setcitestyle{numbers} very good results in the HCI 4D Light Field Benchmark (\cite{hci_benchmark}).\\
  ...
  \begin{appendix}
	...
	where '$*$' describes a convolution.	
  	...
  	\begin{figure}
  		...
  		\caption[Mean squared error for different (3) parameters]{Mean squared error for different (3) parameters.The red points mark the grid points used to create the contour plot. The green dots show the minimum MSE, respectively.}
  	...
  	\begin{figure}
  		...
  		\caption[Discontinuity score for different (3) parameters]{Discontinuity score for different (3) parameters. The red points mark the grid points used to create the contour plot. The green dots show the minimum MSE, respectively.}
  	...
  	\begin{figure}
  		...
  		\caption[Planar score for different (3) parameters]{Planar score for different (3) parameters. The red points mark the grid points used to create the contour plot. The green dots show the minimum MSE, respectively.}
